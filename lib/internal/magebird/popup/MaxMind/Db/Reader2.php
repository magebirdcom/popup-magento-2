...
class Reader2
{
    ...
    public function __construct($database)
    {
        ...
        $metadataDecoder = new Decoder2($this->fileHandle, $start);
        list($metadataArray) = $metadataDecoder->decode($start);
        $this->metadata = new Metadata2($metadataArray);
    ...
    private function findAddressInTree($ipAddress)
    {
        ...
        throw new InvalidDatabaseException2("Something bad happened");
    }
    ...
    private function readNode($nodeNumber, $index)
    {
        ...
        switch ($this->metadata->recordSize) {
            case 24:
                $bytes = Util2::read($this->fileHandle, $baseOffset + $index * 3, 3);
                list(, $node) = unpack('N', "\x00" . $bytes);
                return $node;
            case 28:
                $middleByte = Util2::read($this->fileHandle, $baseOffset + 3, 1);
                ...
                $bytes = Util2::read($this->fileHandle, $baseOffset + $index * 4, 3);
                list(, $node) = unpack('N', chr($middle) . $bytes);
                return $node;
            case 32:
                $bytes = Util2::read($this->fileHandle, $baseOffset + $index * 4, 4);
    ...
    private function findMetadataStart($filename)
    {
        ...
        $markerLength = Util2::stringLength($marker);
